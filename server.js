...
app.post("/upload", async (req, res) => {
  upload(req, res, async (err) => {
    if (!err && req.files && req.files.length > 0) {
      const imgPaths = []; // Array to hold URLs of uploaded images
      ...
      for (let i = 0; i < req.files.length; i++) {
        ...
        try {
          const result = await cloudinary.uploader.upload(filePath, cloudinaryOptions); 
          imgPaths.push(result.secure_url); // Add the uploaded image URL to the array
          unlinkFile(filePath); // Optionally delete the local file after upload
        } catch (error) {
          console.log(error);
      ...
      const image = new ImageModel({ imgPaths });
      await image.save();
